export const SCOPE_FORMAT_RULES = `SCOPE FORMAT: Both scope_description (crew/internal) and client_scope_description MUST be bullet lists, each bullet on its own line starting "- ". No introductory paragraph or run-on paragraph. Client scope: usually 3–6 short bullets in simple language covering the finished work, headline size/material choice and important exclusions. Omit crew instructions, rates, hours and purchasing detail. Crew scope: separate ordered steps expressed as bullets, with confirmed construction details, quantities, sequencing and exclusions. Keep assumptions separate from promised work. Only include exclusions explicitly confirmed by the contractor or supplied project documents. Never invent plant warranty exclusions, warranty durations, or other contractual terms; if missing, leave them unspecified and ask when needed. The client list must be simpler than the crew list without changing the agreed scope.`
