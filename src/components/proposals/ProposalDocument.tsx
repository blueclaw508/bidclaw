...
/**
 * The proposal document — everything between the top of the page and the
 * footer, with no I/O of its own. Two containers render it:
 *
 *   • ProposalPrintView (the contractor's print/PDF page, authenticated)
 *   • ClientProposal   (the client's approval page at /p/:token, public)
 *
 * It was lifted out of ProposalPrintView verbatim when the client page
 * arrived, so the client sees the same document the contractor printed.
 * Nothing in here touches supabase, the router, or auth; the props are
 * the whole contract.
 */
 ...
 */
export type PrintFormat = 'detailed' | 'summary' | 'crew'

/**
 * The slice of company settings the document reads. The public page is
 * handed exactly these columns and nothing else, so the type says so.
 */
...
export function ProposalDocument({
  ...
  format: PrintFormat
  showTotal: boolean
  /** The client's accepted signature, when there is one. Fills the customer signature line. */
  signature?: ProposalSignature | null
}) {
  return (
    <>
      {/* Inline @media print + page-break CSS. Lives with the document so
      ...
      {/* === Customer + Project === */}
      <CustomerProjectBlock project={project} customer={customer} />
...
function ClientClosing({
  ...
  showTotal: boolean
  accent: string
  /** The client's accepted signature, when there is one. */
  signature: ProposalSignature | null
}) {
    ...
    <>
      ...
      <section className="pv-signatures mt-12 grid grid-cols-1 gap-8 sm:grid-cols-2">
        <SignatureLine
          ...
          signature={signature?.decision === 'accepted' ? signature : null}
        />
...
/**
 * Blank ruled line by default. Handed an accepted signature, it prints the
 * drawn signature over the rule with the signer's typed name and the date
 * they signed — the same block on the client's screen, the contractor's
 * print, and the PDF.
 */
  ...
  if (signature?.signature_data) {
    return (
      <div className="pv-signature">
        <div className="mt-2 flex h-16 items-end border-b border-gray-400 pb-1">
          <img
        ...
        <div className="mt-1 flex items-center justify-between text-[10px] uppercase tracking-wider text-gray-500">
          <span>
            {label} · <span className="normal-case tracking-normal text-gray-700">{signature.signer_name}</span>
          </span>
        ...
        <p className="mt-0.5 text-[9px] text-gray-400">
          Signed electronically via BidClaw at{' '}
          {new Date(signature.signed_at).toLocaleTimeString('en-US', {
            hour: 'numeric',
            minute: '2-digit',
            timeZoneName: 'short',
          })}
...
function formatAddress(s: DocumentSettings): string {
