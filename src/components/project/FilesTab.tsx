...
export default function FilesTab({ projectId, onChange }: FilesTabProps) {
  ...
  return (
    ...
    <div className="space-y-4">
                  ...
                  {inGroup.map((file, idx) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button type="button" onClick={()=>{setRename(file);setNewName(file.file_name.replace(/\.[^.]+$/,''))}} className="rounded px-2 py-1 text-sm text-blue-700 hover:bg-blue-50">Rename</button>
      ...
      <Modal open={!!rename} onClose={()=>{if(!renaming)setRename(null)}} title="Rename file" size="md">

        <form onSubmit={async e=>{e.preventDefault();if(!rename||renaming)return;const stem=newName.trim();if(!stem||/[\\/\x00-\x1f]/.test(stem)){toast.error('Enter a file name without slashes or control characters.');return}const ext=rename.file_name.match(/\.[^.]+$/)?.[0]??'';const name=stem.toLowerCase().endsWith(ext.toLowerCase())&&ext?stem:stem+ext;setRenaming(true);try{const {data,error}=await supabase.from('project_files').update({file_name:name}).eq('id',rename.id).eq('project_id',projectId).eq('file_name',rename.file_name).select('*').single();if(error||!data)throw new Error(error?.message??'This file changed. Refresh and retry.');setRows(prev=>prev.map(r=>r.id===data.id?data:r));setRename(null);toast.success('File renamed.');onChange?.()}catch(err){toast.error(err instanceof Error?err.message:'Rename failed.')}finally{setRenaming(false)}}}>

          <label className="block">File name<input autoFocus required maxLength={200} value={newName} onChange={e=>setNewName(e.target.value)} className="mt-2 w-full rounded border p-2"/></label>

          <p className="mt-2 text-sm text-gray-500">The {rename?.file_name.match(/\.[^.]+$/)?.[0]??'original'} file type is preserved.</p>

          <div className="mt-4 flex justify-end gap-3"><button type="button" disabled={renaming} onClick={()=>setRename(null)}>Cancel</button><button disabled={renaming||!newName.trim()} className="rounded bg-brand-navy px-3 py-2 text-white">{renaming?'Saving…':'Save name'}</button></div>
