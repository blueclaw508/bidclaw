...
export default function LeadDetailPage() {
  ...
  const [deleteNote,setDeleteNote]=useState<LeadNote|null>(null)
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_minmax(0,420px)]">
        ...
        <div className="space-y-6">
          ...
          <LeadContacts key={lead.id} leadId={lead.id}/>
            ...
            {(notes ?? []).map((n) => (
              ...
              <li key={n.id} className="rounded-lg border border-brand-border bg-brand-surface px-4 py-3">
                ...
                <div className="flex items-center gap-1.5 text-xs text-brand-text-muted">
                  ...
                  <button type="button" onClick={()=>setDeleteNote(n)} className="ml-auto rounded p-2 text-rose-700 hover:bg-rose-50" aria-label="Delete note" title="Delete note"><Trash2 className="h-4 w-4"/></button>
      ...
      <ConfirmDialog open={!!deleteNote} onClose={()=>setDeleteNote(null)} title="Delete this note?" description={<div><p>This removes the note from this lead.</p><blockquote className="mt-3 max-h-48 overflow-auto whitespace-pre-wrap rounded border p-3">{deleteNote?.body}</blockquote></div>} confirmLabel="Delete note" tone="danger" onConfirm={async()=>{if(!deleteNote)return;const {error}=await supabase.from('lead_notes').delete().eq('id',deleteNote.id).eq('lead_id',lead.id).select('id').single();if(error){toast.error(`Could not delete note: ${error.message}`);return}setNotes(prev=>prev?.filter(n=>n.id!==deleteNote.id)??[]);setDeleteNote(null);toast.success('Note deleted.')}}/>
